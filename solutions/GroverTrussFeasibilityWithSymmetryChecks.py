...
class GroverTrussFeasibility:
    ...
    4. Which element pairs must be symmetric
    """
    ...
    def __init__(self, 
        ...
        symmetric_pairs : List[Tuple[int, int]]
            Element index pairs that must have matching states
            Example: [(0,1), (2,3)] means elem0==elem1 and elem2==elem3
        """
        ...
        n_symmetry_checks = len(symmetric_pairs)
        ...
        self.pair_checks = list(range(self.n_elements + n_constraint_checks,
                                     self.n_elements + n_constraint_checks + n_symmetry_checks))
        self.output_qubit = self.n_elements + n_constraint_checks + n_symmetry_checks
    ...
    def create_oracle(self) -> QuantumCircuit:
        ...
        # 2. Create symmetry check
        sym_qc = self._create_symmetry_circuit()
        qc.compose(sym_qc, inplace=True)
        
        # 3. Final Multi-Controlled X (The "AND" Gate)
        # All constraint checks AND all pair checks must be 1
        all_checks = self.constraint_checks + self.pair_checks
    ...
    def verify_design(self, bitstring: str) -> Dict[str, bool]:
        ...
        all_satisfied = all(node_checks.values()) and symmetry_ok
        
        result = node_checks.copy()
        result['symmetry'] = symmetry_ok
        result['all_satisfied'] = all_satisfied
        
        return result
    

    def get_best_designs(self, counts: Dict[str, int], top_n: int = 1) -> List[Tuple[str, int]]:
