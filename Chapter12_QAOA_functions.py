...
class QAOATrussOptimizerFLRS:
    def __init__(self, fem_model, V_bar, alpha_under=0.9, alpha_over=1.1, 
                 S_under=0.0, S_over=0.05, lambda_penalty=100.0, 
                 p_layers=6, max_iterations=50, params = None, verbose=True):
        """
        QAOA implementation using a FIXED Linear Ramp Schedule.
        This removes the 'minimize' loop to ensure smooth, QA-like convergence.
        """
        self.fem = fem_model #
        self.V_bar = V_bar #
        self.alpha_under = alpha_under #
        ...
        self.n_elements = fem_model.n_elements #
        self.n_qubits = self.n_elements + 1 #
        
        self.Delta_alpha = alpha_over - alpha_under #
        self.Delta_S = S_over - S_under #
        self.E0 = None 

        # Define the Fixed Linear Ramp Schedule
        # gammas (cost) increase while betas (mixer) decrease
        if (params is not None):
            self.fixed_params = params
        else:
            self.gammas = np.linspace(0,1, self.p_layers)
            self.betas = np.linspace(1, 0, self.p_layers)
            self.fixed_params = np.concatenate([self.gammas, self.betas])
    ...
    def formulate_hamiltonian(self, element_energies):
        """Translates the structural QUBO into a QAOA Cost Hamiltonian."""
        if self.E0 is None: self.E0 = np.sum(element_energies) #
    ...
    def optimize(self):
        """Main design loop using FIXED schedule QAOA."""
        sampler = StatevectorSampler() #

        print(f"Starting Fixed-Schedule QAOA Optimization ({self.n_qubits} qubits)...")
        print("Target Volume =", self.V_bar) #
        self.objective_history = [] #
        self.volume_history = [] #
        self.area_history = [] #
        ...
        for it in range(self.max_iterations):
            ...
            # 1. Measurement using the FIXED RAMP
            opt_qc = self.build_circuit(H_cost, self.fixed_params) #
            opt_qc.measure_all() #
            ...
            if self.verbose:
                print(f"Iter {it+1}: Compliance={compliance:.4g}, Vol={vol:.4g}") #
        
            if self.earlyStop and len(self.objective_history) > 1:
                vol_close = abs(volfrac - 1.0) < 0.01 #
                rel_comp = abs(self.objective_history[-1] - self.objective_history[-2]) / max(self.objective_history[-2], 1e-8) #
                if vol_close and rel_comp < 0.005: #
                    if self.verbose: print(f"Early stopping at iteration {it+1}.")
                    break
