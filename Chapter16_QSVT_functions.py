...
def qsp_response_manual(angles, x, signal_operator="Wx"):
    """
    Manually compute QSP response at point x
    
    Args:
        angles: Phase angles [φ_0, φ_1, ..., φ_d]
        x: Point to evaluate at (in [-1, 1])
        signal_operator: "Wx" or "Wz"
    
    Returns:
        Complex polynomial value P(x)
    """
    # Define W(x) matrix based on convention
    if signal_operator == "Wx":
        sqrt_term = np.sqrt(1 - x**2)
        W = np.array([[x, 1j*sqrt_term],
                      [1j*sqrt_term, x]])
        ...
        raise ValueError("signal_operator must be 'Wx' or 'Wz'")
    
    # Build QSP sequence: S(φ_0) W S(φ_1) W ... S(φ_d)
    result = S(angles[0])
    for phi in angles[1:]:
        result = result @ W @ S(phi)
    
    # Return top-left entry (the polynomial value)
    return result[0, 0]


def angles_to_polynomial(angles, signal_operator="Wx"):
    """
    Convert QSP angles to Chebyshev polynomial
    ...
    # Evaluate at Chebyshev nodes for stability
    n_nodes = degree + 1
    nodes = np.cos(np.pi * (2*np.arange(n_nodes) + 1) / (2*n_nodes))
    
    # Evaluate polynomial at each node
    values = np.array([qsp_response_manual(angles, x, signal_operator) 
                       for x in nodes])
    
    # Get Chebyshev coefficients via interpolation
    coefs = np.polynomial.chebyshev.chebinterpolate(
        lambda x: np.interp(x, nodes, values),
        degree
    )
    ...
    # Enforce parity
    if degree % 2 == 0:  # even degree -> even polynomial
        coefs[1::2] = 0
    else:  # odd degree -> odd polynomial
        coefs[0::2] = 0
...
class myQSVT:
    ...
    def get_inverse_phases_sunderhauf(self, kappa, target_error=None):
        ...
        if target_error is not None:
            degree = SunderhaufPolynomial.mindegree_for_error(target_error, a)
